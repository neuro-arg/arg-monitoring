...
CROPPED_WIDTH = int(
    EXPECTED_WIDTH * CROP_RATIO_X - CROP_OFFSET_RATIO_X * EXPECTED_WIDTH
)
CROPPED_HEIGHT = int(
    EXPECTED_HEIGHT * CROP_RATIO_Y - CROP_OFFSET_RATIO_Y * EXPECTED_HEIGHT
)

WHOSE_STREAM_SQUARE_NUMBER = 34
DETECTOR_THRESHOLD = 0.91


def nparray_crop_frame(
    image_array: np.ndarray, real_height: float, real_width: float
) -> np.ndarray:
    """
    Crops the frame
    ...
    """
    return image_array[
        int(CROP_OFFSET_RATIO_Y * real_height) : int(CROP_RATIO_Y * real_height),
        int(CROP_OFFSET_RATIO_X * real_width) : int(CROP_RATIO_X * real_width),
    ]


def nparray_segment_into_squares(
    image_array: np.ndarray, square_size: int
) -> np.ndarray:
    """
    ...
    return (
        ...
        .swapaxes(1, 2)
        .reshape(-1, square_size, square_size, depth)
    )


def nparray_squares_into_frame(
    ...
    """
    return (
        image_array.reshape(29, 29, square_size, square_size, depth)
        .swapaxes(1, 2)
        .reshape(original_height, original_width, depth)
    )


def resize_image(
    image_array: np.ndarray, new_height: int, new_width: int
) -> np.ndarray:
    """
    Resizes an image
...
def create_process_for_720p_video_for_youtube(youtube_url: str) -> subprocess.Popen:
    """
    ...
    if "720p60" not in output:
        ...
        final_code = [segment for segment in output.split("\n") if "720p60" in segment][
            0
        ].split(" ")[0]

    command = (
        f"youtube-dl -f {final_code} -o - '{youtube_url}'"
        " | ffmpeg"
...
def create_process_for_ffmpeg_video(path: str) -> subprocess.Popen:
    ...
    Returns:
        subprocess.Popen: The process
    """
    command = f"ffmpeg -i {path} -vf scale=1280:720 -c:v ppm -f image2pipe -"
...
def ppm_header_parser(
    process: subprocess.Popen, stream: BytesIO, buffer: BytesIO
) -> tuple[int, int, int]:
    """
...
def extract_dynamic_detector_square(frame: np.ndarray, square_size: int) -> np.ndarray:
    """
...
def whose_stream(
    ...
    square_size: int,
) -> tuple[Literal["neuro", "evil", "dunno"], float]:
    """
    ...
    square = target[
        :square_size,
        (WHOSE_STREAM_SQUARE_NUMBER - 1) * square_size : WHOSE_STREAM_SQUARE_NUMBER
        * square_size,
    ]

    tutel_diffs = [
        calculate_rgb_diff(
            (square * (i / 100.0)).astype(np.uint8), tutel_detector_square
        )
        for i in range(100, 170)
    ]
    neuro_diffs = [
        calculate_rgb_diff(
            (square * (i / 100.0)).astype(np.uint8), neuro_detector_square
        )
        for i in range(100, 170)
    ]
    evil_diffs = [
        calculate_rgb_diff(
            (square * (i / 100.0)).astype(np.uint8), evil_detector_square
        )
        for i in range(100, 170)
    ]
    ...
    if (
        tutel_diff > neuro_diff
        and tutel_diff > evil_diff
