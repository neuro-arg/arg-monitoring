...
SourceImageTuple = namedtuple("SourceImageTuple", ["array", "square_number"])
...
def process_squares_with_target_image(
    params: tuple[np.ndarray, SourceImageTuple],
) -> float:
    """
...
def read_one_frame(
    ...
    (width, height, _) = ppm_header_parser(
        process, cast(BytesIO, process.stdout), image_buffer
    )
...
def scrutinize_with_images_and_thresholds(  # pylint: disable=too-many-locals
    ...
    while process.poll() is None and not intent_to_quit:
        ...
        for idx, image in enumerate(images_array):
            if (
                ...
                logging.info(
                    "Detector square recovered after lost for %d seconds",
        ...
        ssim_results_array = [
            map(
                process_squares_with_target_image,
                ((segments, tuple) for tuple in images),
            )
            for images in images_array
        ]
        ssim_scores_array = [
            [max(x, y) for x, y in zip(ssim_scores, ssim_results)]
            for (ssim_scores, ssim_results) in zip(
                ssim_scores_array, ssim_results_array
            )
        ]
    ...
    logging.info(
        "SSIM scores: %s",
        [[float(score) for score in ssim_scores] for ssim_scores in ssim_scores_array],
    )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(SRC_DIRECTORY_NEURO):
        raise FileNotFoundError(f"The directory {SRC_DIRECTORY_NEURO} does not exist.")

    if not os.path.exists(SRC_DIRECTORY_EVIL):
        raise FileNotFoundError(f"The directory {SRC_DIRECTORY_EVIL} does not exist.")

    if not os.path.exists(THRESHOLDS_FILE_NEURO):
        raise FileNotFoundError(f"The file {THRESHOLDS_FILE_NEURO} does not exist.")

    if not os.path.exists(THRESHOLDS_FILE_EVIL):
        raise FileNotFoundError(f"The file {THRESHOLDS_FILE_EVIL} does not exist.")
    ...
    with create_process_for_720p_video_for_youtube(LINK) as PROCESS:
        ...
        DETECTED_STREAMER, ADJUSTMENT_VALUE = whose_stream(
            FIRST_FRAME, tutel_ds, neuro_ds, evil_ds, SQUARE_SIZE
        )
        ...
        IMAGES = load_images_from_directory(
            SRC_DIRECTORY_NEURO if DETECTED_STREAMER == "neuro" else SRC_DIRECTORY_EVIL
        )
        THRESHOLDS = load_thresholds(
            THRESHOLDS_FILE_NEURO
