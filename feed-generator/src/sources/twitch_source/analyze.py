...
def process_squares_with_target_images(
    params: tuple[np.ndarray, list[SourceImageTuple]],
) -> list[float]:
    arr, tuples = params
            ...
            while process.poll() is None:
                ...
                try:
                    (width, height, _) = ppm_header_parser(
                        process, process.stdout, image_buffer
                    )
                ...
                square = image_array[
                    :SQUARE_SIZE,
                    (WHOSE_STREAM_SQUARE_NUMBER - 1)
                    * SQUARE_SIZE : WHOSE_STREAM_SQUARE_NUMBER * SQUARE_SIZE,
                ]
                ds_diff = calculate_rgb_diff(square, ds)
                if ds_diff < DETECTOR_THRESHOLD:
                    logging.debug(
                        "Link %s: Detection square not found, GRACE: %d, Diff: %d",
                        link,
                        grace,
                        ds_diff,
                    )
                    ...
                    if grace >= GRACE_DETECTOR_PERIOD:
                        logging.debug(
                            "Link %s: Detection square not found, quitting", link
                        )
                        break
                else:
                    grace = 0
                ...
                if logging.root.isEnabledFor(logging.DEBUG):
                    ...
                    if time.time() - interval_elapsed >= 5:
                        logging.debug(
                            "Link %s: Speed for this process at %d fps",
                            link,
...
np.savez("results.npz", thresholds=results, raw_results=video_ssim_scores)
print("Results:")
print(results)
print("Means:")
print(mean)
