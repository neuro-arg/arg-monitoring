...
from scrutinize import (
    ...
    load_thresholds,
    read_one_frame,
    scrutinize_with_images_and_thresholds,
...
process = subprocess.Popen(command, shell=True, stdin=sys.stdin, stdout=subprocess.PIPE)
...
if detected_streamers is None:
    detected_streamers = [
        whose_stream(
            first_frame, tutel_detector, neuro_detector, evil_detector, square_size
        )
    ]
...
if detected_streamers[0][0] == "tutel":
    logger.info("Tutel is streaming, no clue expected. Have a good stream!")
    sys.exit(0)
...
for detected_streamer, adj_value in detected_streamers:
    threshold_file = (
        neuro_thresholds if detected_streamer == "neuro" else evil_thresholds
    )
    images_folder = neuro_folder if detected_streamer == "neuro" else evil_folder
    ...
    detector_square = extract_dynamic_detector_square(first_frame, square_size)
...
results = scrutinize_with_images_and_thresholds(
    process, images_array, thresholds_array, detector_squares, adjustment_value
)

for idx, detected_streamer in enumerate(detected_streamers):
    res = json.dumps(results[idx])
    scrutinize_results.append({"streamer": detected_streamer[0], "result": res})
