...
class FrameNavigator:
    def __init__(self, video_path: str, output_dir: str) -> None:
        ...
        self.fig = plt.figure()
        self.button_next = widgets.Button(plt.axes((0.8, 0.025, 0.1, 0.075)), "Next")
        self.button_prev = widgets.Button(
            plt.axes((0.7, 0.025, 0.1, 0.075)), "Previous"
        )
        self.button_save = widgets.Button(plt.axes((0.6, 0.025, 0.1, 0.075)), "Save")
        self.button_skip = widgets.Button(plt.axes((0.4, 0.025, 0.1, 0.075)), "Skip")
        self.square_input_box = widgets.TextBox(
            plt.axes((0.1, 0.025, 0.1, 0.075)), "Square #"
        )
        self.frame_input_box = widgets.TextBox(
            plt.axes((0.3, 0.025, 0.1, 0.075)), "Frame #"
        )
    ...
    def _save(self, event):
        if self.square_input_box.text and self.display_img:
            ...
            assert self.original_img, "Original Image should not be None"
            ...
            while os.path.exists(
                os.path.join(self.out_dir, f"square_{square_idx}_{file_idx}.png")
            ):
                file_idx += 1
    ...
    def _create_mask(self):
        self.mask = np.full(
            (CROPPED_HEIGHT * SCALE, CROPPED_WIDTH * SCALE, 3), 255, dtype=np.uint8
        )
