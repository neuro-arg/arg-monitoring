...
from metadata.ytc_metadata import ChannelInformationGetter
from sources.twitch_source import TwitchSource
from sources.youtube_source import YoutubeSource

logging.basicConfig(
    level=logging.INFO, format="[%(filename)s:%(lineno)s - %(funcName)s()] %(message)s"
)
...
SOUNDCLOUD_FEED_URL = (
    "https://feeds.soundcloud.com/users/soundcloud:users:1258077262/sounds.rss"
)
...
youtube_feed_getter = FeedGetter(
    YOUTUBE_FEED_URL, ["{http://search.yahoo.com/mrss/}community"]
)

soundcloud_feed_getter = FeedGetter(SOUNDCLOUD_FEED_URL, [])
...
if not os.path.exists("cache.json"):
    ...
    with open("cache.json", "r", encoding="ascii") as f:
        ...
        for u, v in zip(
            current_state.to_dict().items(), cached_state.to_dict().items()
        ):
